var roundToPrecision = function(num, precision) {
	if (precision === null) { // Check is precision is totally null
		precision = 10;
	}
	...
	function AffineTransformation(from, to) {
  	...
  	AffineTransformation.prototype.transformation_matrix = function() {
		...
		for (j = _i = 0, _ref = this.dim; 0 <= _ref ? _i < _ref : _i > _ref; j = 0 <= _ref ? ++_i : --_i) {
	  		this.matrix[j] = [];
	  		for (i = _j = 0, _ref1 = this.dim; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
				this.matrix[j].push(roundToPrecision(this.m[i][j + this.dim + 1]));
	  		}
	  		this.matrix[j].push(roundToPrecision(this.m[this.dim][j + this.dim + 1]));
		}
		return this.matrix;
  	};
